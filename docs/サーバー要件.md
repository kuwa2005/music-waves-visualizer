...
サブディレクトリ（例: `/visualizer`）に配置する場合の簡易手順：
...
`visualizer/` ディレクトリの中身をサーバーの `visualizer` フォルダにアップロードし、`https://あなたのドメイン/visualizer/` でアクセス可能です。詳細は `visualizer/README.txt`（ビルド後に生成）を参照してください。

### 3.2 詳細セットアップ（カスタム設定時）
...
### 3.6 共有ホスティングでのNode.js実行（Node.js対応サーバーの場合）
